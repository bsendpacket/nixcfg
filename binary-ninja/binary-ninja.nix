{ lib, channels, binaryNinjaUrl ? null, binaryNinjaHash ? null, pythonEnv }:
...
let
  ...
  binaryNinja = channels.nixpkgs-unstable.stdenv.mkDerivation rec {
    pname = "binary-ninja";
    version = "dev";
    name = "${pname}-${version}";
    ...
    installPhase = ''
      ...
      export LD_LIBRARY_PATH="$out/opt/binaryninja/binaryninja:${channels.nixpkgs-unstable.lib.makeLibraryPath buildInputs}:\$LD_LIBRARY_PATH"
