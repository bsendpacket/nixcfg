{ channels, binaryNinjaUrl ? null, binaryNinjaHash ? null, }:
...
let
  ...
  # }
  binaryNinjaZip = channels.nixpkgs-unstable.fetchurl {
  ...
  binaryNinjaAPI = channels.nixpkgs-unstable.python312Packages.buildPythonPackage rec {
    ...
    nativeBuildInputs = with channels.nixpkgs-unstable; [
      unzip
      autoPatchelfHook
      patchelf
    ];
    ...
    installPhase = ''
      ...
      patchelf --set-rpath ${channels.nixpkgs-unstable.stdenv.cc.cc.lib}/lib $out/lib/python3.12/libbinaryninjaui.so.1
    '';

    meta = with channels.nixpkgs-unstable.lib; {
